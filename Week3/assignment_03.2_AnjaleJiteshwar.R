...
ggplot(acs_df, aes(HSDegree)) + geom_histogram(bins = 10) + ggtitle("HSDegree vs Frequnecy") + xlab("HSDegree(%)") +ylab("Frequnecy") 
...
## As the probability distribution in given histogram has a single peak, the data distribution unimodal.
...
ggplot(acs_df, aes(x=HSDegree)) + geom_histogram(aes(y = ..density..),bins = 10)+ stat_function(fun = dnorm, colour = "red",args = list(mean = mean(acs_df$HSDegree, na.rm = TRUE), sd = sd(acs_df$HSDegree, na.rm = TRUE))) + ggtitle("HSDegree vs Frequnecy") + xlab("HSDegree(%)") +ylab("Frequnecy") 
