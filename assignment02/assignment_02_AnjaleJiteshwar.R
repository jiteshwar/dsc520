...
person_df2 <- read.csv('data/tidynomicon/person.csv',stringsAsFactors=TRUE)
str(person_df2)
...
voter_turnout_df1 <- readxl::read_excel('data/G04ResultsDetail2004-11-02.xls',sheet ='Voter Turnout',skip = 1)
...
voter_turnout_df2 <- readxl::read_excel('data/G04ResultsDetail2004-11-02.xls',sheet='Voter Turnout',skip = 2,col_names = c("ward_precint", "ballots_cast", "registered_voters", "voter_turnout"))
...
person_df <- DBI::dbGetQuery(db,"SELECT * FROM PERSON;")
head(person_df)
